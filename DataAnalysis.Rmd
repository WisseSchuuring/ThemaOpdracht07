...
DEGData <- MyData[gene.names[pVec < alpha],]
cat("Number of DEGs:\n")
...
```{r}
DEGData$avg_p <- rowMeans(DEGData[1:3])
DEGData$avg_KO <- rowMeans(DEGData[4:6])


DEG_avg <- DEGData[c(7,8)]
avg.fpm <- log2( (DEG_avg / (colSums(DEGData) / 1e6)) + 1 )
avg.fpm
p1 <- hist(avg.fpm$avg_p, col="Orange")
p2 <- hist(avg.fpm$avg_KO, col="Blue")
